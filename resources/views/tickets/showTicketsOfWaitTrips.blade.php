...
<script>
    $(document).ready(function() {
        var table = $('#ShowTicketsTable').DataTable({
            ...
            ajax: '{{ route('showWaitTripTickets', $id) }}',
            columns: [
                ...
                {  data: 'id',
                    ...
                    render: function(data, type, row) {
                        return `
                  <a  href="{{ route('ConfirmPassengerAttendance', '')}}/${data}" class="btn btn-success">تأكيد</a>
                    `;
                    }
                },
                {
                    ...
                    render: function(data, type, row) {
                        return `
                            <form action="{{ route('cancelTicket', '') }}/${data}" method="POST" style="display: inline;">
                                @csrf
                        @method('DELETE')
                        <button type="submit"  onclick="return confirm('هل أنت متأكد من حذف التذكرة؟')"  class="btn btn-danger" style=" position: relative ;right: 10px;">حذف</button>
                    </form>
                          `;
                    }
                },
...
@section('titleOfPage','التذاكر')

@section('title','التذاكر')

@section('titleOfBox','جميع التذاكر ')
    ...
    <div class="container" >
        ...
        <table id="ShowTicketsTable" class="table table-striped nowrap" style="width:500px ;font-size:medium  ">
            ...
            <tr>
                ...
                <th>نوع الحجز</th>
                <th>رقم الرحلة</th>
                <th>تأكيد حضور الراكب</th>
                <th>إلغاء حجز تذكرة</th>
